...
<div style={{ maxWidth: "860px", margin: "0 auto", padding: "40px 20px 80px" }}>
  ...
  <Entry date="11 set 2026" label="Alteração" labelColor="#F59E0B">
    <h2 id="calculo-individual-por-entrega" style={{ fontSize: "22px", fontWeight: "700", marginBottom: "6px", marginTop: 0 }}>
      Categorias com cálculo individual por entrega
    </h2>
    <p style={{ fontSize: "15px", opacity: 0.7, marginBottom: "20px", lineHeight: "1.6" }}>
      A central passou a oferecer um modo de tarifação em que cada entrega é cobrada pelo trecho
      coleta &rarr; entrega, e o valor do pedido é a soma dessas entregas — em vez da tarifa aplicada
      uma vez sobre a rota otimizada. Esse modo depende de valores que só a tela de gestão de pedidos
      calcula, então <strong>a criação pela API não o suporta</strong>. Os endpoints abaixo mudam de
      comportamento apenas quando a categoria está configurada assim.
    </p>

    <EndpointBadge method="POST" path="/api/v2/integracao/entregas" href="/pages/v2/entregas/endpoint/post" />

    <ChangeSection type="added">
      <p style={{ marginBottom: "10px" }}>Abrir entrega em categoria com cálculo individual passa a ser recusado com <code>400</code> e <code>139 CATEGORIA_CALCULO_INDIVIDUAL_NAO_SUPORTADA</code>.</p>
      <ul style={{ margin: 0, paddingLeft: "20px", display: "flex", flexDirection: "column", gap: "8px" }}>
        <li>Vale tanto para a categoria informada no corpo quanto para a que vem da configuração da integração.</li>
        <li>Mesmo comportamento em <code>/entregas/programadas</code>, <code>PUT /entregas/programadas/{"{id}"}</code> e nos três endpoints de estimativa.</li>
        <li>Corridas de passageiro <strong>não</strong> são afetadas: o modo é exclusivo de entregas.</li>
      </ul>
    </ChangeSection>

    <EndpointBadge method="POST" path="/api/v2/integracao/entregas/estimativas" href="/pages/v2/entregas/endpoint/post-estimativas" />

    <ChangeSection type="added">
      <p style={{ margin: 0 }}>Estimativa nessas categorias devolve o mesmo <code>139</code>. O valor calculado aqui seria menor que o cobrado, então a estimativa é recusada em vez de responder um número que não se confirma no disparo. Na versão multicategorias da v1, as categorias com cálculo individual são <strong>omitidas da resposta</strong> e a chamada segue devolvendo <code>200</code> com as demais.</p>
    </ChangeSection>
    ...
    <ChangeSection type="added">
      <p style={{ margin: 0 }}>Novo erro <code>140 ESTIMATIVA_ENTREGA_AUSENTE</code> (HTTP <code>400</code>) quando alguma entrega da programada não tem estimativa individual gravada. Sem ela o valor restante não pode ser recalculado, e a exclusão é recusada em vez de gravar um total errado.</p>
    </ChangeSection>

    <EndpointBadge method="POST" path="/api/v2/integracao/entregas/{id}/agrupar" href="/pages/v2/entregas/endpoint/post-agrupar" />

    <ChangeSection type="changed">
      <p style={{ marginBottom: "10px" }}>Quando as entregas do grupo usam cálculo individual, o valor do grupo passa a ser a <strong>soma das estimativas de cada entrega</strong>, e não a tarifa sobre a rota mesclada. Retorno, dinâmica e desconto continuam incidindo uma única vez sobre o total.</p>
      <ul style={{ margin: 0, paddingLeft: "20px", display: "flex", flexDirection: "column", gap: "8px" }}>
        <li>O <code>422</code> por entrega do grupo sem estimativa individual gravada agora tem código próprio, <code>141 ESTIMATIVA_ENTREGAS_INCONSISTENTE</code>, em vez do <code>0</code> genérico — dá para tratá-lo programaticamente. Somar apenas as entregas que têm estimativa cobraria menos que o total real.</li>
        <li>Vale também para <code>/entregas/{"{id}"}/agrupar/programadas</code> e <code>/entregas/programadas/{"{id}"}/agrupar</code>.</li>
      </ul>
    </ChangeSection>
    ...
    <div style={{
      ...
      <strong>Nada muda para quem não usa o modo.</strong> Categorias com a tarifação de sempre seguem
      com o comportamento atual em todos os endpoints. A configuração é feita pela central, por
      categoria de tarifa — consulte o time da Gaudium antes de ativá-la em uma integração.
    </div>
  </Entry>
