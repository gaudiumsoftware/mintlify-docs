...
<div style={{ maxWidth: "860px", margin: "0 auto", padding: "40px 20px 80px" }}>
  ...
  <Entry date="1 set 2026" label="Alteração" labelColor="#F59E0B">
    ...
    <p style={{ fontSize: "15px", opacity: 0.7, marginBottom: "20px", lineHeight: "1.6" }}>
      ...
      calcula, então <strong>a API de integração não o suporta</strong>. Os endpoints abaixo mudam de
      comportamento apenas quando a categoria está configurada assim.
    </p>

    <EndpointBadge method="POST" path="/abrirSolicitacao" href="/pages/v1/entregas/solicitacao/endpoint/post-abrirSolicitacao" />

    <ChangeSection type="changed">
      <p style={{ marginBottom: "10px" }}>Abrir entrega em categoria com cálculo individual passa a ser recusado com <code>139 CATEGORIA_CALCULO_INDIVIDUAL_NAO_SUPORTADA</code>.</p>
    ...
    <EndpointBadge method="GET" path="/estimarSolicitacao" href="/pages/v1/entregas/solicitacao/endpoint/get-estimarSolicitacao" />

    <ChangeSection type="changed">
      <p style={{ margin: 0 }}>Estimativa de entrega nessas categorias devolve o mesmo <code>139</code>. O valor calculado aqui seria menor que o cobrado, então a estimativa é recusada em vez de responder um número que não se confirma no disparo.</p>
    </ChangeSection>

    <EndpointBadge method="GET" path="/estimarSolicitacaoMulticategorias" />

    <ChangeSection type="changed">
      <p style={{ margin: 0 }}>Categorias com cálculo individual são <strong>omitidas da resposta</strong>, pelo mesmo motivo. A chamada continua devolvendo <code>200</code> com as demais categorias.</p>
    </ChangeSection>

    <EndpointBadge method="DELETE" path="/api/v2/integracao/entregas/{id}/paradas/{parada_id}" />

    <ChangeSection type="added">
      <p style={{ margin: 0 }}>Novo erro <code>139</code> quando a entrega a excluir não tem estimativa individual gravada. Sem ela o valor restante não pode ser recalculado, e a exclusão é recusada em vez de gravar um total errado.</p>
    </ChangeSection>
    ...
    <ChangeSection type="changed">
      ...
      <ul style={{ margin: 0, paddingLeft: "20px", display: "flex", flexDirection: "column", gap: "8px" }}>
        <li>Novo motivo de <code>422</code>: alguma entrega do grupo sem estimativa individual gravada. Somar apenas as que têm cobraria menos que o total real.</li>
