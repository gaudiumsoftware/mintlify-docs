...
<div style={{ maxWidth: "860px", margin: "0 auto", padding: "40px 20px 80px" }}>
  ...
  <Entry date="29 set 2026" label="Novo" labelColor="#3b82f6">
    <h2 id="retorno-dados-fechamento-entrega" style={{ fontSize: "22px", fontWeight: "700", marginBottom: "6px", marginTop: 0 }}>
      Retorno e dados de fechamento na consulta da entrega
    </h2>
    <p style={{ fontSize: "15px", opacity: 0.7, marginBottom: "20px", lineHeight: "1.6" }}>
      A consulta da entrega passou a retornar o endereço de retorno e os dados de fechamento que
      antes só vinham no recibo. Assim, você não precisa chamar o endpoint de recibo separadamente.
    </p>

    <EndpointBadge method="GET" path="/api/v2/integracao/entregas/{id}" href="/pages/v2/entregas/endpoint/get-by-id" />

    <ChangeSection type="added">
      <ul style={{ margin: 0, paddingLeft: "20px", display: "flex", flexDirection: "column", gap: "8px" }}>
        <li><ParamBadge name="desejado" /> — endereço de retorno (<ParamBadge name="endereco" />, <ParamBadge name="bairro" />, <ParamBadge name="cidade" />, <ParamBadge name="estado" />, <ParamBadge name="lat" /> e <ParamBadge name="lng" />). Vem <code>null</code> quando a entrega não tem retorno, o mesmo critério de <ParamBadge name="com_retorno" />. Os destinos continuam em <ParamBadge name="paradas" />.</li>
        <li><ParamBadge name="data_hora_inicio" /> — início da entrega. Vem <code>null</code> enquanto não iniciou. Segue o padrão dos demais campos <code>data_hora_*</code> da consulta: horário de Brasília, no formato <code>AAAA-MM-DD HH:MM:SS</code>. O recibo traz esse horário em <ParamBadge name="partida.data_hora" />, em ISO 8601 e no fuso da central (o <code>Z</code> no fim não indica UTC).</li>
        ...
        <li><ParamBadge name="nome_cliente" /> e o documento: <ParamBadge name="cpf_cliente" /> quando da entrega não é corporativa, ou <ParamBadge name="cnpj_cpf_empresa" /> quando é (com <ParamBadge name="empresa_id" />).</li>
        <li><ParamBadge name="codigo_cupom" /> e <ParamBadge name="gerador_cupom_id" /> — só quando da entrega usou cupom.</li>
      </ul>
    </ChangeSection>

    <div style={{ marginTop: "24px" }}>
      <EndpointBadge method="POST" path="/api/v2/integracao/entregas/consultar" href="/pages/v2/entregas/endpoint/post-consultar" />

      <ChangeSection type="added">
        <ul style={{ margin: 0, paddingLeft: "20px", display: "flex", flexDirection: "column", gap: "8px" }}>
          <li>Os mesmos campos, em cada entrega da lista retornada.</li>
        </ul>
      </ChangeSection>
    </div>
    ...
    <div style={{
      ...
      <strong>Nota:</strong> os campos vêm direto na raiz de cada entrega, em qualquer status, e não dentro de um bloco <code>recibo</code>. Os dados de condutor, veículo, valores, duração e distância já eram retornados e não mudaram.
    </div>
  </Entry>
  ...
  <Entry date="17 set 2026" label="Novo" labelColor="#3b82f6">
    ...
    <ChangeSection type="added">
      <ul style={{ margin: 0, paddingLeft: "20px", display: "flex", flexDirection: "column", gap: "8px" }}>
        <li><ParamBadge name="paradas[].url_confirmacao" /> — opcional. Ao informá-la, o app passa a pedir o código ao condutor e, na confirmação, chamamos a URL com <code>POST</code> e corpo JSON contendo <code>codigo_confirmacao</code>, <code>solicitacao_id</code>, <code>solicitacao_parada_id</code> e <code>id_externo</code>. Responda <code>2xx</code> para confirmar a entrega e <code>4xx</code> para recusar o código.</li>
        <li>Com a URL informada, o <code>codigo_confirmacao</code> da parada deixa de ser necessário — quem valida o código é o seu sistema, não nós.</li>
      </ul>
    </ChangeSection>
    ...
    <ChangeSection type="changed">
      <ul style={{ margin: 0, paddingLeft: "20px", display: "flex", flexDirection: "column", gap: "8px" }}>
        <li>O <code>codigo_confirmacao</code> agora está documentado como um código de <strong>4 dígitos</strong> (de <code>0001</code> a <code>9999</code>).</li>
        <li>O <strong>&ldquo;prosseguir sem código&rdquo;</strong> da plataforma, antes restrito a pedidos do iFood, passa a atender também as entregas abertas com <code>url_confirmacao</code> — nos detalhes da solicitação e na gestão de pedidos.</li>
      </ul>
    </ChangeSection>
  ...
  <Entry date="13 ago 2026" label="Novo" labelColor="#3b82f6">
    ...
    <div style={{
      ...
      <strong>Nota:</strong> o parâmetro é absoluto sobre as duas configurações do cadastro da empresa — vale mesmo com <em>Solicitar código de confirmação de entrega dos pedidos</em> e <em>Tornar os códigos de confirmação de entrega obrigatórios</em> desabilitadas. Ele só soma exigência: nunca dispensa a que já está configurada. Em contrapartida, o <code>codigo_confirmacao</code> de cada parada passa a ser obrigatório — sem ele o condutor não teria o que informar para concluir a entrega, e a solicitação é recusada com <code>400 CODIGO_CONFIRMACAO_OBRIGATORIO</code> (código 136). Pedidos do iFood continuam exigindo o código independente do valor enviado, por ser regra da própria integração. A exigência criada pelo parâmetro não é removível depois: o <em>prosseguir sem código</em> da plataforma atende só pedidos do iFood, e a edição da entrega não altera o parâmetro. O código fica visível no painel da empresa, no link de acompanhamento e em <code>GET /entregas/{id}/links-rastreio</code>.
    </div>
  </Entry>
  ...
  <Entry date="13 ago 2026" label="Melhoria" labelColor="#16A34A">
    ...
    <ChangeSection type="added">
      ...
      <ul style={{ margin: 0, paddingLeft: "20px", display: "flex", flexDirection: "column", gap: "8px" }}>
        ...
        <li><ParamBadge name="paradas" /> — lista de paradas na ordem da entrega, cada uma com <code>id</code>, <code>ordem</code>, endereço completo, <code>numero_pedido</code>, <code>nome_cliente</code>, <code>telefone_cliente</code> e <code>observacao</code>.</li>
      </ul>
    </ChangeSection>
