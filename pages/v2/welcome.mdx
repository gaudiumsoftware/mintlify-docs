...
```php PHP
$baseUrl = 'https://api.sua-central.com.br';
$apiKey = 'sua-api-key';
...
$href = str_replace('{id}', $dinamicas['dados'][0]['id'], $acaoAtualizar['href']);
...
```java Java
String baseUrl = "https://api.sua-central.com.br";
...
String href = acaoAtualizar.path("href").asText()
    .replace("{id}", dinamicas.path("dados").get(0).path("id").asText());
...
dados := dinamicas["dados"].([]any)
primeiraDinamica := dados[0].(map[string]any)
...
```ruby Ruby
base_url = 'https://api.sua-central.com.br'
api_key = 'sua-api-key'
...
href = acao_atualizar['href'].sub('{id}', dinamicas['dados'][0]['id'].to_s)
...
const baseUrl = 'https://api.sua-central.com.br';
...
const href = acaoAtualizar.href.replace('{id}', dinamicas.dados[0].id);
...
acao_atualizar = dinamicas['__links']['atualizar-dinamica-area']
href = acao_atualizar['href'].replace('{id}', str(dinamicas['dados'][0]['id']))
