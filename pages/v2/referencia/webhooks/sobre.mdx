...
| `type_code` | ENUM | SIM | Tipo da conversa ( `P`, `E`) |
| `type_label` | String | SIM | Rótulo do tipo da conversa ( `DRIVER`, `ENTERPRISE`) |
...
```json
{
 ...
 "event_id": "0199a3e2-7f10-7c3a-9b2d-4e5f6a7b8c9d",
 "message_id": 91011,
 "request_id": 752894625,
 "sender_id": 321,
 ...
 "type_label": "RIDE",
 "links": {
   "sender": "https://api.taximachine.com.br/api/v2/integracao/clientes/321",
   "request": "https://api.taximachine.com.br/api/integracao/solicitacao?id=752894625"
 }
}
```
...
| Campo | Formato | Requerido | Descrição |
| :--- | :--- | :--- | :--- |
| `datetime` | ISO-8601 | SIM | Data e hora da mensagem. |
| `event_id` | UUID | SIM | Identificação única do evento (idempotência). |
| `company_id` | INT | SIM | Identificador da central |
| `message_id` | INT | SIM | Identificador da mensagem |
| `request_id` | INT | SIM | Identificador da solicitação (corrida) a que a conversa pertence |
| `sender_id` | INT | SIM | Identificador do remetente: id do condutor ou id do cliente, conforme `sender_type` |
| `sender_type` | ENUM | SIM | Quem enviou a mensagem ( `DRIVER`, `PASSENGER`) |
| `sequence` | INT | SIM | Número sequencial da mensagem na conversa da corrida |
| `type_code` | ENUM | SIM | Tipo da conversa. Sempre `S` neste webhook |
| `type_label` | String | SIM | Rótulo do tipo da conversa. Sempre `RIDE` neste webhook |
| `content` | String | SIM | Conteúdo textual da mensagem |
| `links` | Object | SIM | URLs relacionadas ao evento: o remetente ( `sender`) e a solicitação ( `request`) |

### Enum `sender_type`

| Valor | Descrição | `sender_id` | `links.sender` |
| :--- | :--- | :--- | :--- |
| `DRIVER` | Mensagem enviada pelo condutor | Identificador do condutor | `/api/integracao/condutor?id={id}` |
| `PASSENGER` | Mensagem enviada pelo passageiro | Identificador do cliente | `/api/v2/integracao/clientes/{id}` |

### Enum `type_code`

| Code | Label | Descrição |
| :--- | :--- | :--- |
| `S` | `RIDE` | Chat da corrida entre condutor e passageiro |
