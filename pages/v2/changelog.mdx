...
<div style={{ maxWidth: "860px", margin: "0 auto", padding: "40px 20px 80px" }}>
  ...
  <Entry date="29 set 2026" label="Novo" labelColor="#3b82f6">
    <h2 id="destino-dados-fechamento-corrida" style={{ fontSize: "22px", fontWeight: "700", marginBottom: "6px", marginTop: 0 }}>
      Destino e dados de fechamento na consulta da corrida
    </h2>
    <p style={{ fontSize: "15px", opacity: 0.7, marginBottom: "20px", lineHeight: "1.6" }}>
      A consulta da corrida passou a retornar o endereço de destino e os dados de fechamento que
      antes só vinham no recibo. Assim, você não precisa chamar o endpoint de recibo separadamente.
    </p>
    ...
    <ChangeSection type="added">
      <ul style={{ margin: 0, paddingLeft: "20px", display: "flex", flexDirection: "column", gap: "8px" }}>
        <li><ParamBadge name="desejado" /> — endereço de destino (<ParamBadge name="endereco" />, <ParamBadge name="bairro" />, <ParamBadge name="cidade" />, <ParamBadge name="estado" />, <ParamBadge name="lat" /> e <ParamBadge name="lng" />). Vem <code>null</code> quando a corrida não tem destino definido.</li>
        <li><ParamBadge name="data_hora_inicio" /> — início da corrida. Vem <code>null</code> enquanto não iniciou. Segue o padrão dos demais campos <code>data_hora_*</code> da consulta: horário de Brasília, no formato <code>AAAA-MM-DD HH:MM:SS</code>. O recibo traz esse horário em <ParamBadge name="partida.data_hora" />, em ISO 8601 e no fuso da central (o <code>Z</code> no fim não indica UTC).</li>
        <li><ParamBadge name="tipo_pagamento" /> e <ParamBadge name="desconto" />. O desconto é o percentual aplicado e vem <code>0.00</code> quando não há.</li>
        <li><ParamBadge name="tarifa" />, <ParamBadge name="id_categoria" />, <ParamBadge name="categoria" /> e <ParamBadge name="descricao_categoria" />.</li>
        <li><ParamBadge name="nome_cliente" /> e o documento: <ParamBadge name="cpf_cliente" /> quando da corrida não é corporativa, ou <ParamBadge name="cnpj_cpf_empresa" /> quando é (com <ParamBadge name="empresa_id" />).</li>
        <li><ParamBadge name="codigo_cupom" /> e <ParamBadge name="gerador_cupom_id" /> — só quando da corrida usou cupom.</li>
      </ul>
    </ChangeSection>
    ...
    <div style={{ marginTop: "24px" }}>
      ...
      <ChangeSection type="added">
        <ul style={{ margin: 0, paddingLeft: "20px", display: "flex", flexDirection: "column", gap: "8px" }}>
          <li>Os mesmos campos, em cada corrida da lista retornada.</li>
        </ul>
      </ChangeSection>
    </div>
    ...
    <div style={{
      ...
      <strong>Nota:</strong> os campos vêm direto na raiz de cada corrida, em qualquer status, e não dentro de um bloco <code>recibo</code>. Os dados de condutor, veículo, valores, duração e distância já eram retornados e não mudaram.
    </div>
  </Entry>
