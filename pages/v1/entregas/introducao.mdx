...
| -------- | -------- | -------- | -------- |
| Crítico | `/api/integracao/solicitacao`<br />`/api/integracao/solicitacaoStatus` | `api-key` | 800 * fator requisições por minuto |
| Crítico por integrador externo | `/api/integracao/solicitacao`<br />`/api/integracao/solicitacaoStatus` | `int-key` | 800 * fator requisições por minuto |
| Condutores e créditos | `/api/integracao/saldoCreditosCondutor`<br />`/api/integracao/saldoCreditosCondutor/`<br />`/api/integracao/condutor`<br />`/api/integracao/taxista` | `api-key` | 80 * fator requisições por minuto |
| Posição do condutor | `/api/integracao/posicaoCondutor` | `api-key` | 1000 * fator requisições por minuto |
| Categorias | `/integracao/v1/categoria*` | `api-key` | Sem bloqueio configurado. O gateway registra log a partir de 10 * fator requisições por minuto. |
| Bandeiras | `/integracao/v1/bandeira*` | `api-key` | Sem bloqueio configurado. O gateway registra log a partir de 10 * fator requisições por minuto. |
| Saque automático | `/integracao/v1/saqueAutomatico*` | `api-key` | 50 * fator requisições por minuto |
| Programadas | `/api/integracao/consultarProgramada` | `api-key` | 200 * fator requisições por minuto |
| Dinâmica | `/integracao/v1/dinamica*` | `api-key` | 200 * fator requisições por minuto |
| In-app messaging | `/integracao/v1/notificacao/inAppMessaging` | `api-key` | 100 * fator requisições por minuto |
| Padrão | `/api/integracao/*` | `api-key` | 1000 * fator requisições por minuto |
